    ...
    render() {
        return (
                <tr id={this.props.id}>
                  ...
                  <td className="numbers">£{(this.props.state[this.props.coin] * this.props.price / this.props.exchangeRate).toFixed(2)}</td>     
                  <td><button className="table_buttons" onClick={() => this.props.removeCoin(this.props.id)}>remove</button></td>    
                </tr>
        );
  }
}
